...
$(function(){
	$.get("house/loaddistricts",function(data){
		...
		$.each(data, function(i, item){
			var option = $("<option></option>").attr("value",item.id).html(item.name);
	...
	$("#district").change(function(){
		...
		$.get("house/loadstreets",{"districtId": districtId},function(data){
			var select = $("#street");
			$.each(data, function(i, item){
				var option = $("<option></option>").attr("value",item.id).html(item.name);
				select.append(option);
...
<script language="javascript">
	...
	<table align="center">
	<tr>
		<td width=''><font color='red'>当前用户：${sessionScope.loginUser.name} </font>
		</td>
	</tr>
	<tr>
    ...
    <form action="house/list" method="post" name="mf">
		...
		<table width="88%;" id="advSearch" style="border:solid 0px #000;display:none;">
			...
			<tr>
				<td>租金：</td>
				<td><input type="text" name="lowPrice" size="5" class="unnamed1" > 至 <input type="text" name="hiPrice" size="5" class="unnamed1" > 元/月</td>
			</tr>
			<tr>
				<td colspan='3'><hr/></td>
			</tr>
					...
					<c:if test="${not empty housePage}">
						 ...
						 <c:if test="${housePage.pageNo gt 3}">
						  ...
						  <c:if test="${housePage.pageNo-2 ge 1}">
						    <li><a href="house/list.do?pageNo=${housePage.pageNo-2}">${housePage.pageNo-2}</a></li>
						  </c:if>
						  <c:if test="${housePage.pageNo-1 ge 1}">
						    <li><a href="house/list.do?pageNo=${housePage.pageNo-1}">${housePage.pageNo-1}</a></li>
						  </c:if>
						    <li  class="active"><a href="house/list.do?pageNo=${housePage.pageNo}">${housePage.pageNo}</a></li>
						 ...
						 <c:if test="${housePage.pageNo+2 lt housePage.totalPage}">
						    ...
						    <li><a href="house/list.do?pageNo=${housePage.totalPage}">»</a></li>
						  </ul>
					</c:if>
					</td>
				</tr>
